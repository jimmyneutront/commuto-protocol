...
class CommutoRaiseDisputeTest(CommutoSwapTest.CommutoSwapTest):
    ...
    def test_raiseDispute_dispute_agents_different_check(self):
        #Ensure raiseDispute tests that all three dispute agents are different
        newOfferID = HexBytes(uuid4().bytes)
        tx_details = {
            "from": self.maker_address
        }
        ...
        try:
            ...
            raise (Exception("test_raiseDispute_dispute_agents_different_check failed without raising exception"))
        except ValueError as e:
